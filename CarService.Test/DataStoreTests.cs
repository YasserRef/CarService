...
using Xunit;
using ValueType = JsonFileData.ValueType;
...
namespace CustomerService.Test
{
    public class DataStoreTests
    {
        ...
        public async Task WriteToFile_LowerCamelCase()
        {
            ...
            var collection = store.GetCollection("PrivateOwner");
            Assert.Equal(0, collection.Count);

            await collection.InsertOneAsync(new { FirstName = "Jimmy", OwnerLongTestProperty = "UT" });
            Assert.Equal(1, collection.Count);
            ...
            var collectionUppercase = store2.GetCollection("PrivateOwner");
            Assert.Equal(0, collectionUppercase.Count);

            var collectionLowercase = store2.GetCollection("privateOwner");
            Assert.Equal(1, collectionLowercase.Count);

            //var collectionNoCase = store2.GetCollection();
        ...
        public void GetCollection_TypedCollection_NameParameter()
        {
            ...
            var collection = store.GetCollection("Invoice");
        ...
        {
            var innerCollection = new DocumentCollection<Invoice>(
                new Func<string, Func<List<Invoice>, bool>, bool, Task<bool>>((s, d, i) => Task.FromResult(true)),
                new Lazy<List<Invoice>>(() => new List<Invoice> { new Invoice { Name = "Commando" } }),
                @"/path/to/file",
                "id",
                new Func<Invoice, Invoice>(m => m),
                new Func<Invoice>(() => new Invoice()));

            var store = Substitute.For<IDataStore>();
            store.GetCollection<Invoice>().Returns(innerCollection);

            var collection = store.GetCollection<Invoice>();
            Assert.Equal(1, collection.Count);

            var result = collection.InsertOne(new Invoice { Name = "Predator" });
            Assert.True(result);
            Assert.Equal(2, collection.Count);
            Assert.Single(collection.AsQueryable().Where(e => e.Name == "Predator"));
        }
        ...
        {
            var innerCollection = new DocumentCollection<Invoice>(
                new Func<string, Func<List<Invoice>, bool>, bool, Task<bool>>((s, d, i) => throw new Exception("Failed")),
                new Lazy<List<Invoice>>(() => new List<Invoice> { new Invoice { Name = "Commando" } }),
                @"/path/to/file",
                "id",
                new Func<Invoice, Invoice>(m => m),
            ...
            Assert.Throws<Exception>(() => collection.InsertOne(new Invoice { Name = "Predator" }));
        }
        ...
        public async Task Readme_Example()
        {
            ...
            var typedCollection = store.GetCollection<Customer>();

            var userTyped = typedCollection
                                .AsQueryable()
                                .Single(p => p.Name == "Phil");

            typedCollection.InsertOne(new Customer { Id = 15, Name = "Jim", Age = 52 });
            typedCollection.ReplaceOne(e => e.Id == 15, new Customer { Id = 15, Name = "Barry", Age = 52 });
        ...
        public async Task FileContent_DefaultFormat(bool useLowerCamelCase)
        {
            ...
            var collection = store.GetCollection<CarService>("CarService");
            await collection.InsertOneAsync(new CarService { Name = "Test", Cost = 555});
        ...
        public void FileContent_AlwaysMinify_Encrypted()
        {
            ...
            storeFileEncrypted.InsertItem<CarService>("CarService", new CarService { Name = "Matrix", Cost = 5 });
            var content = UTHelpers.GetFileContent(path);
